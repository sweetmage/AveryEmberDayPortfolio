...
> ### Status, 2026-08-10: partly superseded — read alongside LOGBOOK Entry 131
>
> This plan was written on a `develop` tree that was **8 commits behind `portfoliowebsite`**, where
> the same wedge had already been fixed on 2026-08-09. Production's mechanism shipped and this
> plan's **Fix B was dropped**: it rescues on *lack of progress* (`NO_PROGRESS_FRAMES`) instead of
> on elapsed frames, which separates a genuine wedge from the deliberate 8px/frame escape glide
> without any threshold guesswork.
>
> **Fix A, the seed-clear, did ship** (Entry 133) and is complementary rather than competing: it
> stops 2–3 of the 7 global bubbles being *born* inside a zone on every load, so the rescue has
> fewer wedges to catch and none during the first frames. The regression spec shipped too.
>
> The measurements below stand and are worth keeping: they were taken independently of Entry 131's
> and agree with them closely (67 consecutive overlap frames here, 68 there, `_relocating` FALSE in
> both), which is why the root cause can be treated as settled rather than as one session's theory.
