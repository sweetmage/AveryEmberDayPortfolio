      ...
      test.describe(`${page.name} @ ${width}px — ${theme}`, () => {
        ...
        test('visual baseline', async ({ page: p }) => {
          ...
          /* Then wait for the DOCUMENT HEIGHT to stop moving.
             ...
             `useStickyRailOverlay` publishes the pinned strip's height to
             `--brand-rail-overlay` from a `useEffect` + ResizeObserver, i.e.
             AFTER first paint, and `--stage-cap` / `--art-cap` are computed
             from it — so the Mistrust stage and the gallery art resize once
             more after everything above has already settled. Caught 2026-08-10
             as `projects-mistrust @ 768px — light` failing with
             "Failed to take two consecutive stable screenshots", 125,968 pixels
             different, while passing 3/3 on immediate re-run.

             Height rather than a specific token, deliberately: this catches any
             late reflow, including the next one nobody has thought of, and it
             does not need to know which pages have a rail. `scrollHeight` is
             integer, so equality is safe. */
          await p.waitForFunction(
            () => {
              const h = document.documentElement.scrollHeight;
              const prev = window.__lastStableHeight;
              window.__lastStableHeight = h;
              window.__stableCount = prev === h ? (window.__stableCount || 0) + 1 : 0;
