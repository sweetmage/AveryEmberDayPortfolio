...
>
> Their **layout** comes from `images/myart/A History of Mistrust/frame-geometry.json`, the committed manifest of where each frame sits on the Figma canvas. **Re-read those coordinates whenever frames are moved or resized in Figma, and commit the manifest with the re-export.** A resize is caught (every slide PNG is asserted against its manifest entry, in both the generator and `tests/mistrust-sets.spec.js`), and so are gaps and out-of-order offsets. A frame *translated* without being resized is caught by nothing — no pixel inspection can see it — so the manifest is only as fresh as the last time someone read it. Until 2026-08-14 offsets were instead recovered by template-matching each slide into the raster set exports, which made every build depend on exports the user's Figma export has never included (Entry 113), so any repaint of a slide blocked the build. The three `Set N.png` files are kept as source-of-record only.
>
