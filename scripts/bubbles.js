...
(function () {
  ...
  const SQUISH_AMOUNT   = 0.20; // max deformation on impact
  const MIN_SPEED       = 0.4;
      ...
      for (let i = 0; i < this.blobs.length; i++) {
        for (let j = i + 1; j < this.blobs.length; j++) {
          ...
          if (dist < minDist && dist > 0.1) {
            ...
            a.scaleX = 1 - 0.1 * Math.abs(c);
            a.scaleY = 1 + 0.1 * 0.5 * Math.abs(s);
            b.scaleX = 1 - 0.1 * Math.abs(c);
            b.scaleY = 1 + 0.1 * 0.5 * Math.abs(s);
          }
        }
      }
    }
