...
- `docs/plans/2026-07-05-tailwind-restoration.md` — Tailwind v4 pipeline restored (2026-07-05), then **merged with remediation-2 (`896dba3`) on 2026-07-09**: bubble physics, Playwright suite, metadata recovered. Squish/bounce retuned (softer spring, 0.7 restitution); zone-tracker refresh and trapped-bubble deadlock fixed; `package-lock.json` committed; `node_modules/` untracked. ✅ Complete.
- `docs/plans/2026-07-09-tailwind-utility-conversion.md` — Nav trimmed to Work + About (no submenu/Contact/CTA/hamburger); **all 5 pages converted to Tailwind utility authoring**; `brand.css` compiled into the components layer (single `style.css`); dead submenu/hamburger CSS+JS removed. ✅ Complete.
...
- `docs/plans/2026-07-02-project-card-bubble-exclusion.md` — `.bubble-exclude` marker class added to project cards + `.about-box`; `DEFAULT_EXCLUSIONS` now includes it and the hardcoded `#work .project-card` / `#about .about-box` entries were removed. **Status:** ✅ Complete (2026-07-10).
